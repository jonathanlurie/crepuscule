var u = Object.defineProperty;
var L = (d, t, i) => t in d ? u(d, t, { enumerable: !0, configurable: !0, writable: !0, value: i }) : d[t] = i;
var s = (d, t, i) => (L(d, typeof t != "symbol" ? t + "" : t, i), i);
import { addProtocol as b } from "@maptiler/sdk";
const a = "", S = typeof window < "u" && window.Blob && new Blob([atob(a)], { type: "text/javascript;charset=utf-8" });
function r() {
  let d;
  try {
    if (d = S && (window.URL || window.webkitURL).createObjectURL(S), !d)
      throw "";
    return new Worker(d);
  } catch {
    return new Worker("data:application/javascript;base64," + a);
  } finally {
    d && (window.URL || window.webkitURL).revokeObjectURL(d);
  }
}
const y = "crepuscule_protocole_<UNIQUE>", n = "crepuscule_source_<UNIQUE>", m = "crepuscule_layer_<UNIQUE>", K = {
  color: [0, 0, 17],
  opacity: 0.7,
  date: /* @__PURE__ */ new Date(),
  debug: !1
};
class c {
  constructor(t, i = {}) {
    s(this, "map");
    s(this, "color");
    s(this, "opacity");
    s(this, "date");
    s(this, "unique");
    s(this, "protocolNamespace");
    s(this, "tileUriPattern");
    s(this, "layerId");
    s(this, "sourceId");
    s(this, "debug");
    s(this, "source");
    const e = {
      ...K,
      ...i
    };
    this.map = t, this.color = e.color.slice(), this.opacity = e.opacity, this.date = e.date, this.debug = e.debug, this.unique = (Math.random() + 1).toString(36).substring(2), this.protocolNamespace = y.replace(
      "<UNIQUE>",
      this.unique
    ), this.tileUriPattern = `${this.protocolNamespace}://{z}-{x}-{y}-${+this.date}`, this.layerId = m.replace("<UNIQUE>", this.unique), this.sourceId = n.replace(
      "<UNIQUE>",
      this.unique
    );
  }
  async generateTilePixelOnWorker(t, i, e, l) {
    return new Promise((h) => {
      const o = new r();
      o.postMessage({
        x: t,
        y: i,
        z: e,
        timestamp: l,
        color: this.color,
        debug: this.debug
      }), o.onmessage = (p) => {
        h(p.data), o.terminate();
      };
    });
  }
  init() {
    b(
      this.protocolNamespace,
      (t, i) => {
        if (!t.url)
          throw new Error("");
        const [e, l, h, o] = t.url.split("/").pop().split("-").map((p) => parseFloat(p));
        return this.generateTilePixelOnWorker(l, h, e, o).then((p) => {
          i(null, p, null, null);
        }), { cancel: () => {
        } };
      }
    ), this.map.addSource(this.sourceId, {
      type: "raster",
      tiles: [this.tileUriPattern],
      tileSize: 512
    }), this.source = this.map.getSource(this.sourceId), this.map.addLayer({
      id: this.layerId,
      type: "raster",
      source: this.sourceId,
      paint: {
        // @ts-ignore
        "raster-opacity-transition": { duration: 1e3, delay: 0 },
        "raster-opacity": this.opacity
      }
    });
  }
  setOpacity(t, i = {}) {
    this.opacity = t, this.map.setPaintProperty(this.layerId, "raster-opacity-transition", {
      duration: 0,
      delay: 0,
      ...i
    }), this.map.setPaintProperty(this.layerId, "raster-opacity", t, {
      validate: !1
    });
  }
  hide(t = {}) {
    this.setOpacity(0, t);
  }
  show(t = {}) {
    this.setOpacity(this.opacity, t);
  }
  setDate(t) {
    this.date = t, this.tileUriPattern = `${this.protocolNamespace}://{z}-{x}-{y}-${+this.date}`, this.source.tiles[0] = this.tileUriPattern, this.source.load();
  }
  update() {
    this.setDate(/* @__PURE__ */ new Date());
  }
}
class k {
  constructor(t, i = {}) {
    s(this, "opacity");
    s(this, "crA");
    s(this, "crB");
    s(this, "usingA");
    s(this, "intervalId");
    const e = {
      ...K,
      ...i
    };
    this.opacity = e.opacity, e.debug ? (this.crA = new c(t, {
      ...e,
      color: [70, 0, 0]
    }), this.crB = new c(t, {
      ...e,
      opacity: 0,
      color: [0, 0, 70]
    })) : (this.crA = new c(t, e), this.crB = new c(t, { ...e, opacity: 0 })), this.usingA = !0, this.intervalId = null;
  }
  init() {
    this.crA.init(), this.crB.init(), this.start();
  }
  start() {
    this.intervalId = setInterval(() => {
      this._update();
    }, 5e3);
  }
  stop() {
    clearInterval(this.intervalId), this.intervalId = null;
  }
  _update() {
    const t = this.usingA ? this.crA : this.crB, i = this.usingA ? this.crB : this.crA;
    this.usingA = !this.usingA, i.setDate(/* @__PURE__ */ new Date()), t.setOpacity(0, { duration: 0, delay: 1e3 }), i.setOpacity(this.opacity, { duration: 0, delay: 1e3 });
  }
}
export {
  c as Crepuscule,
  k as CrepusculeLive
};
